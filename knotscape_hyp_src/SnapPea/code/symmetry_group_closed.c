	...
	Boolean			*is_full_group)
{
	 ...
	 */
	{
		...
		if (compute_cusped_symmetry_group(manifold, NULL, &dummy, symmetry_group) == func_OK)
		{
	...
	Triangulation		**symmetric_triangulation)
{
	...
	{
		if (compute_cusped_symmetry_group(manifold, NULL, &manifold_sym_grp, &link_sym_grp) != func_OK)
		{
			free_triangulation(manifold);
			return;
		}
	 ...
	 */
	if (compute_cusped_symmetry_group(manifold, NULL, &manifold_sym_grp, &link_sym_grp) == func_failed)
	{
		free_triangulation(manifold);
		return;
	}
