 ...
 *	of calling conventions.  An exception occurs when using Symantec C++
 *	in conjunction with ANSI library routines which take function pointers
 *	as arguments.  Specifically, qsort() wants a comparision function
 *	declared using C calling conventions.  Symantec's convention is to
 *	use the keyword _cdecl ("C declaration") to indicate that a function
 *	should be compiled with C calling conventions.  So we define CDECL
 *	to be _cdecl when compiling under Symantec C++ for Macintosh, and
 *	null otherwise.
 */
