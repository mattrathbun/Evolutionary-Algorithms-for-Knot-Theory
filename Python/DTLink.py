# Any benefit to encoding DT code as a dictionary {1:e_1, 3:e_3, ... }
# instead of a list?
...
class DTLink(object):
  ...
  # the even number belongs to understrand or the overstrand of the diagram,
  # respectively.
  def triple(arc):
    code = self.code
  ...
  def R1UpPlus(self, arc):
    ...
    for i in range(n + 1):
      ...
      # and change the (even) code by 2 if it is after the strand to be changed
      # (arc/idx) (keeping the same sign).
      if (over < idx):
        # code at position i
	    ci = self.code[i]
	  ...
	  # For over indices that equal the strand to be changed (necessarily odd),
	  # insert the next even number at that index in the code.
      elif (over == idx):
	    new_dt.append(idx + 1)
      # For over indices that are one more than the (even) strand to be
      # changed, insert the (even) strand to be changed at that index in the
      # code.
      elif (over == idx + 1):
	    new_dt.append(idx)
	  ...
	  # (arc/idx) (keeping the same sign).
        ...
        # Code index of the original code (shifted forward by one list position
        # in the new code)
	    ci = self.code[i - 1]
	    # Sign of the code in the original code (shifted forward by one list
	    # position in the new code)
	    cis = cmp(ci, 0)
	    # Absolute value of the code index of the original code (shifted forward
	    # by one list position in the new code)
	    cia = abs(ci)
  ...
  # Method mutates the DTLink object, and returns True if move is
  # successfully performed (which it should, because there is no
  # obstruction).
  ...
  # a (positive) even number in the code corresponds to an adjacent (odd)
  # number.)
