...
class Fit(object):
    ...
    def __call__(self, ol):
        if ol.fitness == -float('inf'):
            ...
            if d.number_crossings() < 3:
                bonus = 10000
                fitness = 1.0 + bonus/(d.number_crossings()**float(1) + ccCount**float(3) + ol.length()**float(0) + (ol.length() - min_ol.length())**float(2) + 1.0)
            else:
                bonus = 1
                fitness = 1.0 + bonus/(d.number_crossings()**float(2) + ccCount**float(1) + ol.length()**float(0) + (ol.length() - min_ol.length())**float(2) + 1.0)
            ol.setFitness(fitness)
            return fitness
        elif isinstance(ol.fitness, float):
            return ol.fitness
