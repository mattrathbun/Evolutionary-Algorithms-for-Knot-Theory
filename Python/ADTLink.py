...
#   	return True	
  ...
  #wibble: unused?
	...
	# Helper: returns the position of a crossing in the code list
	def crossPos(self,arc):
		if (self.isOdd(arc)):
			return (arc-1)/2
		else:
			return abs(self.jump(arc)-1)/2
  ...
  ##wibble: need to remember that there is a special case when
	...
	def R3(self, arc, side):
  	...
  	if side in ['R', 'r', 'right', '1', 'Right']:
			...
			crossedOverOrientation2 = self.orientations[self.crossPos(self.right(arc))]
			if self.isOdd(self.right(arc)):
				# print "crossing", (self.right(arc)-1)/2
				crossedOverOrientation = self.orientations[(self.right(arc)-1)/2]
			else:
				# print "crossing ",(self.right(arcNext)-1)/2
				crossedOverOrientation = self.orientations[(self.right(arcNext)-1)/2]
				print "crossedOverOrientation ", crossedOverOrientation, crossedOverOrientation2
			#wibble deal with the crossing
